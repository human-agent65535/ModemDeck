...
CREATE TABLE modemdeck_apple_push_deliveries (
			...
			FOREIGN KEY (event_key) REFERENCES modemdeck_notification_events(event_key) ON DELETE CASCADE ON UPDATE CASCADE,
			FOREIGN KEY (credential_id) REFERENCES modemdeck_ios_pairing_credentials(id) ON DELETE CASCADE ON UPDATE CASCADE
		);
...
CREATE INDEX idx_modemdeck_apple_push_pending
	ON modemdeck_apple_push_deliveries(
		status, next_attempt_at, expires_at, created_at, event_key, credential_id
	);
